...
def construct_dataset(clearml_info_string):
    ...
    assert set(
        ...
        {"train", "val", "val", "nc", "names"}
    ), "The right keys were not found in the yaml file, make sure it at least has the following keys: ('train', 'val', 'val', 'nc', 'names')"
    ...
    def __init__(self, opt, hyp):
        ...
        if self.clearml:
            ...
            if opt.data.startswith("clearml://"):
                ...
                # names, nc (number of classes), val, train, val (all three relative paths to ../datasets)
