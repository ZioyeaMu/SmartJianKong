...
def run(
    ...
    task="val",  # train, val, val, speed or study
    ...
    Args:
        ...
        task (str, optional): Task type - 'train', 'val', 'val', 'speed', or 'study'. Default is 'val'.
    ...
    if not training:
        ...
        task = task if task in ("train", "val", "val") else "val"  # path to train/val/val images
...
def parse_opt():
    ...
    Args:
        ...
        max_det (int, optional): Maximum number of detections per image. Default is 300.
        task (str, optional): Task type - options are 'train', 'val', 'val', 'speed', or 'study'. Default is 'val'.
    ...
    parser.add_argument("--task", default="val", help="train, val, val, speed or study")
...
def main(opt):
    ...
    if opt.task in ("train", "val", "val"):  # run normally
        ...
        if opt.task == "speed":  # speed benchmarks
            ...
            raise NotImplementedError(f'--task {opt.task} not in ("train", "val", "val", "speed", "study")')
