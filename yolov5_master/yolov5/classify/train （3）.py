...
def train(opt, device):
    ...
    test_dir = data_dir / "val" if (data_dir / "val").exists() else data_dir / "val"  # data/val or data/val
    if RANK in {-1, 0}:
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and ("backbone.layer4" in name or "backbone.layer3" in name):
            eca_layer = ECALayer(k_size=5)  # 增大 k_size 捕获长距离依赖
            parent_name = ".".join(name.split(".")[:-1])
            parent_module = model.get_submodule(parent_name) if parent_name else model
            child_name = name.split(".")[-1]
            new_module = nn.Sequential(module, eca_layer)
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        # Log metrics
        if RANK in {-1, 0}:
            train_losses.append(tloss)  # 记录训练损失
            val_losses.append(vloss)    # 记录验证损失
    ...
    if RANK in {-1, 0} and final_epoch:
        ...
        plt.plot([float(loss) for loss in train_losses], label='Training Loss')  # 确保转换为标量
        plt.plot([float(loss) for loss in val_losses], label='Validation Loss')  # 确保转换为标量
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
...
def parse_opt(known=False):
    ...
    parser.add_argument("--data", type=str, default="D:/PyCharm/SmartJianKong/yolov5_master/datasets/my_datas", help="cifar10, cifar100, mnist, imagenet, ...")
    ...
    parser.add_argument("--name", default="traffic_exp1", help="save to project/name")
    ...
    parser.add_argument("--label-smoothing", type=float, default=0.01, help="Label smoothing epsilon")
